...
Everything needs a unique tuple or list, although it also works with a simple value. Since tuples
are the norm, that will be assumed in examples. The tuple defines the attribute that you are
generating.
If you only want one of the attribute per random seed, you only need one entry in the tuple, or a
simple value.
If you do not need a seed, then you can use the pghash class directly:
    pgh = pghash.pghash
The pghgen wrapper just stores the seed and constructs a new tuple consisting of the seed plus the
tuple supplied.
...
Changing the seed will, obviously, change the values generated by the hashing. Changing the lambda
...
The default lambda converts the tuple to a string with the default str function, which should be
suitable for most purposes.

Here are some alternative joiners, which will be used to generate a stringification lambda:
